...
本地生成 / 识别二维码，支持样式定制与可扫描性自检。

| 功能 | 说明 |
|------|------|
| 内容类型 | 文本 / URL、WiFi、vCard 名片 |
| 基础样式 | 前景/背景色、纠错等级、预设与自定义尺寸、安全留白 |
| 外框装饰 | 开关、宽度、颜色、圆角、背景、内边距（与安全留白分离） |
| Logo | 本地上传 PNG/JPG/WebP，大小/圆角/裁剪，自动建议 H 纠错 |
| 扫描自检 | 生成后本地回读，提示对比度、留白、Logo 等风险 |
| 导出 | PNG / SVG（含外框与 Logo），历史记录本地保存 |

### ✍️ Markdown 创作台

纯前端 Markdown 编辑环境：导入、实时预览、本地草稿与安全导出。

| 功能 | 说明 |
|------|------|
| 编辑模式 | 仅编辑 / 左右分屏 / 仅预览 / 沉浸式全屏 |
| 导入 | `.md` `.markdown` `.txt` `.html`，拖拽或选择文件 |
| 工具栏 | 标题、加粗、列表、任务、代码块、表格等 |
| 大纲与统计 | H1–H6 大纲跳转，字符/词数/阅读时间 |
| 本地草稿 | IndexedDB 自动保存，最近文档列表 |
| 导出 | `.md` / 安全 HTML，复制 Markdown 或 HTML |
